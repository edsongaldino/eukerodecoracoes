...
if(decodifica($_POST["acao"]) == "adicionar-noticia"){
    ...
    // Insere o produto
    $noticia = $pdo->prepare("INSERT INTO noticia 
                            (titulo, resumo, texto, arquivo, autor, data, status)
                            VALUES
                            (:titulo, :resumo, :texto, :arquivo, :autor, :data, :status)");

    $noticia->execute(array(
        ':titulo' => $titulo,
        ':resumo' => $resumo,
        ':texto' => $texto,
        ':arquivo' => $filename,
    ...
    if(!$noticia){
        $error_log[] = $noticia->errorInfo();
    }

    if($error_log){
        redireciona("/sistema/sessao_noticia/consultar.php?me=".codifica(1,true)."&mm=".codifica("Ops, algo deu errado na inclusão da noticia!")); 
    }else{
        redireciona("/sistema/sessao_noticia/consultar.php?me=".codifica(0,false)."&mm=".codifica("A noticia foi adicionada com sucesso!")); 
    }
}
                ...
                <div class="container-fluid">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card">
                                <form method="post" action="sessao_noticia/adicionar.php" class="form-horizontal" multipart="" enctype="multipart/form-data">
                                    <div class="card-header card-header-text" data-background-color="rose">
                                        <h4 class="card-title">DADOS DO noticia</h4>
                                    </div>
                                    <div class="card-content">
                                        <?php require_once("form.php");?>
                                    </div>
                                    <input type="hidden" name="acao" id="acao" value="<?php echo codifica("adicionar-noticia"); ?>">
                                    <input type="hidden" name="codigo_noticia" id="codigo_noticia" value="<?php echo codifica($codigo_noticia); ?>">
                                    <button type="submit" class="btn btn-fill btn-rose salvar">Gravar noticia</button>
